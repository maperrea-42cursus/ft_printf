...
/*   Updated: 2020/02/06 19:25:36 by maperrea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	(tag.flags & HASHTAG) && tag.specifier == HEX_LC ? write(1, "0x", 2) : 0;
...
int		print_hex(t_tag tag, va_list ap)
{
	...
	print_nbr_hex(tag, n);
	free(n.nbr);
	return (n.width > n.size ? n.width : n.size + (tag.flags & HASHTAG));
}
